...
if(isset($_POST['submit'])){
    ...
    $ch=mysqli_query($con,"select * from bank_info where otp='$otp' and bank_id='$bid'");
    $ch2=mysqli_fetch_array($ch);
    $anu="select * from payment where reg_id='$pd' and magazine='$mg'";
    $s=mysqli_query($con,$anu);
    $ui=mysqli_fetch_array($s);
    $id=$ui['id'];
    
    $no=mysqli_num_rows($ch);
    if($no>0){
    $balance=$ch2['balance'];
    $debit=$balance-$amount;
 $ch11=mysqli_query($con,"select * from bank_info where name='ADMIN'");
    $ch21=mysqli_fetch_array($ch11);
        $new=$ch21['balance'];
        $credit=$new+$amount;
        
        if($balance>0){
      $qw=mysqli_query($con,"update bank_info set otp='0' where bank_id='$bid'");
    $qw2=mysqli_query($con,"update bank_info set balance='$debit' where bank_id='$bid'");
    $qw3=mysqli_query($con,"update bank_info set balance='$credit' where name='ADMIN'");

     $qw5=mysqli_query($con,"update payment set status='paid' where id='$id'");
        }else{ ?>
   ...
   <?php }
     ...
     $to = "archanakartha16@gmail.com";
        ...
        $message = "Dear Customer, \n you had successfully subsribed for the book,it will be delivered to you for the subscribed months .\n  Thanks and Regards,\nAgrom Team";
        $headers = "From:".$from;     
      ...
      header("location:magazinecart.php");
}
              ...
              <div class="col-lg-6">
                <div class="contact-box">
                  <div class="section-title">
                    <h2>Enter Your <span>OTP</span></h2>
                        ...
                        <div class="controls">
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                   ...
                                   <label>OTP</label>
                                    <input type="password" name="otp" class="form-control customize" />
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <p> <input type="submit" class="btn btn-custom" name="submit"  onclick="return validate()" value="Proceed"></p>
                                </div>
                            </div>
                        </div>
                    </form>  
